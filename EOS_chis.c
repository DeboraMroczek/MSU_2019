...
int main(int argc, char *argv[])
{
	...
	char nameCoords[100],nameFolder[100],nameMainfolder[100],nameChisIsing[100],nameChisNoIsing[100],namedChisNoIsingdT[100],named2ChisNoIsingdT2[100],
			namePressIsing3D[100],namedPdTIsing3D[100],namedPdmuBIsing3D[100],named2PdT2Ising3D[100],named2PdmuB2Ising3D[100], named4PdmuB4Ising3D[100], named2PdTdmuBIsing3D[100],
			namePressNoIsing3D[100],namedPdTNoIsing3D[100],namedPdmuBNoIsing3D[100],named2PdT2NoIsing3D[100],named2PdmuB2NoIsing3D[100],named2PdTdmuBNoIsing3D[100],
			namePressTotHRG3D[100],namedPdTTotHRG3D[100],namedPdmuBTotHRG3D[100],named2PdT2TotHRG3D[100],named2PdmuB2TotHRG3D[100],named2PdTdmuBTotHRG3D[100],			
			namePressIsingPlusNoIsing3D[100],namedPdTIsingPlusNoIsing3D[100],namedPdmuBIsingPlusNoIsing3D[100],named2PdT2IsingPlusNoIsing3D[100],
			named2PdmuB2IsingPlusNoIsing3D[100],named2PdTdmuBIsingPlusNoIsing3D[100],namePressFinal3D[100],nameEnerDensFinal3D[100],nameBarDensFinal3D[100],nameEntrFinal3D[100],
			nameSpsoundFinal3D[100],nameChi2Final3D[100],nameChi4Final3D[100];
	...
	d3PressNoIsingFilterdmuB3Mat=matrix(5,821,0,601); d4PressNoIsingFilterdmuB4Mat=matrix(5,821,0,601);

	PressLATonlyMat=matrix(5,821,0,601);			dPressLATonlydTMat=matrix(5,821,0,601);				dPressLATonlydmuBMat=matrix(5,821,0,601);	
	d2PressLATonlydT2Mat=matrix(5,821,0,601);		d2PressLATonlydmuB2Mat=matrix(5,821,0,601);			d2PressLATonlydTdmuBMat=matrix(5,821,0,601);
	d3PressLATonlydmuB3Mat=matrix(5,821,0,601);		d4PressLATonlydmuB4Mat=matrix(5,821,0,601);			PressLATonlyFilterMat=matrix(5,821,0,601);

	PressLATonlyNormMat=matrix(5,821,0,601);		EntropyLATonlyNormMat=matrix(5,821,0,601);			BarDensityLATonlyNormMat=matrix(5,821,0,601);	
	EnerDensityLATonlyNormMat=matrix(5,821,0,601);	SpSoundLATonlyNormMat=matrix(5,821,0,601);			Chi2LATonlyNormMat=matrix(5,821,0,601);
	Chi4LATonlyNormMat=matrix(5,821,0,601);	
	...
	d3PressIsingdmuB3Mat=matrix(5,821,0,601);		d4PressIsingdmuB4Mat=matrix(5,821,0,601);
	...
	d3PressTotdmuB3Mat=matrix(5,821,0,601);			d4PressTotdmuB4Mat=matrix(5,821,0,601);	
	...
	d3PressHRGdmuB3Mat=matrix(5,821,0,601);			d4PressHRGdmuB4Mat=matrix(5,821,0,601);	
	...
	d3PressTotHRGdmuB3Mat=matrix(5,821,0,601);		d4PressTotHRGdmuB4Mat=matrix(5,821,0,601);
	...
	Chi4FinalMat=matrix(5,821,0,601);
...
// ---------------------------- START LATTICE CASE ----------------------------------//
		...
		for (i=5; i<=821; i++){
			for(j=0; j<=601; j++){
		    	muBval = (double) j; 	Tval = (double) i;
       			PressLATonlyMat[i][j] = Chi0LatVec[i]+(1.0/2.0)*Chi2LatVec[i]*pow(muBval/Tval,2)
    												+(1.0/24.0)*Chi4LatVec[i]*pow(muBval/Tval,4);
   			}
		}

		for (i=5; i<=820; i++)  for (j=0;j<=601; j++)	dPressLATonlydTMat[i][j] = PressLATonlyMat[i+1][j] - PressLATonlyMat[i][j]; 
		for (i=5; i<=821; i++)	for (j=0;j<=600; j++)	dPressLATonlydmuBMat[i][j] = PressLATonlyMat[i][j+1] - PressLATonlyMat[i][j];
		for (i=5; i<=819; i++)  for (j=0;j<=601; j++)	d2PressLATonlydT2Mat[i][j] = dPressLATonlydTMat[i+1][j] - dPressLATonlydTMat[i][j]; 
		for (i=5; i<=821; i++)  for (j=0;j<=599; j++)	d2PressLATonlydmuB2Mat[i][j] = dPressLATonlydmuBMat[i][j+1] - dPressLATonlydmuBMat[i][j];
		for (i=5; i<=820; i++)  for (j=0;j<=600; j++)	d2PressLATonlydTdmuBMat[i][j] = dPressLATonlydTMat[i][j+1] - dPressLATonlydTMat[i][j];
		for (i=5; i<=821; i++)  for (j=0;j<=598; j++)	d3PressLATonlydmuB3Mat[i][j] = d2PressLATonlydmuB2Mat[i][j+1] - d2PressLATonlydmuB2Mat[i][j];
		for (i=5; i<=821; i++)  for (j=0;j<=597; j++)	d4PressLATonlydmuB4Mat[i][j] = d3PressLATonlydmuB3Mat[i][j+1] - d3PressLATonlydmuB3Mat[i][j];
	...
	for(i=5;i<=820;i++){
		for(j=0;j<=601;j++){
			...
			/*NOTE: File output is currently disabled. Uncomment file export line to turn on file export. */
	...
	//FILE *FiledPdTNoIsingFilter = fopen("NoIsingdPdTFilter.dat","w");				FILE *FiledPdmuBNoIsingFilter = fopen("NoIsingdPdmuBFilter.dat","w");
	//FILE *Filed2PdT2NoIsingFilter = fopen("NoIsingd2PdT2Filter.dat","w");			FILE *Filed2PdmuB2NoIsingFilter = fopen("NoIsingd2PdmuB2Filter.dat","w");
	//FILE *Filed2PdTdmuBNoIsingFilter = fopen("NoIsingd2PdTdmuBFilter.dat","w");		FILE *Filed3PdmuB3NoIsingFilter = fopen("NoIsingd3PdmuB3Filter.dat","w");
	...
	for (i=5; i<=821; i++)  for (j=0;j<=598; j++)	d3PressNoIsingFilterdmuB3Mat[i][j] = d2PressNoIsingFilterdmuB2Mat[i][j+1] - d2PressNoIsingFilterdmuB2Mat[i][j];
	for (i=5; i<=821; i++)  for (j=0;j<=597; j++)	d4PressNoIsingFilterdmuB4Mat[i][j] = d3PressNoIsingFilterdmuB3Mat[i][j+1] - d3PressNoIsingFilterdmuB3Mat[i][j];
...
//---------------MERGING WITH HRG BEYOND THIS POINT---------------------------//
	...
	for (i=5; i<=820; i++)  for (j=0;j<=601; j++)	dPressHRGdTMat[i][j] = PressHRGMat[i+1][j] - PressHRGMat[i][j]; 
	for (i=5; i<=821; i++)  for (j=0;j<=600; j++)	dPressHRGdmuBMat[i][j] = PressHRGMat[i][j+1] - PressHRGMat[i][j]; 

	for (i=5; i<=819; i++)  for (j=0;j<=601; j++)	d2PressHRGdT2Mat[i][j] = dPressHRGdTMat[i+1][j] - dPressHRGdTMat[i][j]; 
	for (i=5; i<=821; i++)  for (j=0;j<=599; j++)	d2PressHRGdmuB2Mat[i][j] = dPressHRGdmuBMat[i][j+1] - dPressHRGdmuBMat[i][j]; 
	for (i=5; i<=820; i++)  for (j=0;j<=600; j++)	d2PressHRGdTdmuBMat[i][j] = dPressHRGdTMat[i][j+1] - dPressHRGdTMat[i][j]; 

	//	for (i=5; i<=821; i++)  for (j=0;j<=598; j++)	d3PressHRGdmuB3Mat[i][j] = d2PressHRGdmuB2Mat[i][j+1] - d2PressHRGdmuB2Mat[i][j]; 

	//	for (i=5; i<=821; i++)  for (j=0;j<=597; j++)	d4PressHRGdmuB4Mat[i][j] = d3PressHRGdmuB3Mat[i][j+1] - d3PressHRGdmuB3Mat[i][j]; 
	...
	free_matrix(PressLATonlyMat,5,821,0,601);			free_matrix(PressLATonlyFilterMat,5,821,0,601);		free_matrix(dPressLATonlydTMat,5,821,0,601);		
	free_matrix(dPressLATonlydmuBMat,5,821,0,601);		free_matrix(d2PressLATonlydT2Mat,5,821,0,601);	free_matrix(d2PressLATonlydmuB2Mat,5,821,0,601);	
	free_matrix(d2PressLATonlydTdmuBMat,5,821,0,601);		free_matrix(d3PressLATonlydmuB3Mat,5,821,0,601);	free_matrix(d4PressLATonlydmuB4Mat,5,821,0,601);

	free_matrix(PressLATonlyNormMat,5,821,0,601);		free_matrix(EntropyLATonlyNormMat,5,821,0,601);		free_matrix(BarDensityLATonlyNormMat,5,821,0,601);
	free_matrix(EnerDensityLATonlyNormMat,5,821,0,601);	free_matrix(SpSoundLATonlyNormMat,5,821,0,601);		free_matrix(Chi2LATonlyNormMat,5,821,0,601);
	free_matrix(Chi4LATonlyNormMat,5,821,0,601);
	...
	free_matrix(d3PressNoIsingFilterdmuB3Mat,5,821,0,601);	free_matrix(d4PressNoIsingFilterdmuB4Mat,5,821,0,601);
	...
	free_matrix(d3PressIsingdmuB3Mat,5,821,0,601);	    free_matrix(d4PressIsingdmuB4Mat,5,821,0,601);
	...
	free_matrix(d3PressTotdmuB3Mat,5,821,0,601);		free_matrix(d4PressTotdmuB4Mat,5,821,0,601);
	...
	free_matrix(d3PressHRGdmuB3Mat,5,821,0,601);		free_matrix(d4PressHRGdmuB4Mat,5,821,0,601);
	...
	free_matrix(d3PressTotHRGdmuB3Mat,5,821,0,601);	    free_matrix(d4PressTotHRGdmuB4Mat,5,821,0,601);
	...
	free_matrix(Chi4FinalMat,5,821,0,601);
